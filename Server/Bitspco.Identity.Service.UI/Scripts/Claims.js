...
var datatable = $('#grid').sigmaDataTable({
    ...
    columns: [
        ...
        { data: "ModuleId", text: 'شناسه ماژول' },
        { data: "Module", text: 'ماژول', render(row) { return row["Module"].Name } },
        { data: "Name", text: 'نام' },
        { data: "Symbol", text: 'نماد' },
        {
            data: "Type", text: 'نوع', width: '150px', type: 'select',
            render(row) { return ClaimTypes[row.Type] ? ClaimTypes[row.Type].Value : '-' },
            filter: {
                items: ClaimTypes
            }
        },
        {
            ...
            render(row) { return ClaimStatuses[row.Status] ? ClaimStatuses[row.Status].Value : '-' },
            filter: {
                items: ClaimStatuses
            }
        },
        {
            text: '', width: '137px', type: 'control', render(row) {
...
document.getElementById('create').onshow = function (args) {
    if (!this.app) {
        this.app = new Vue({
            el: `#${this.id} .body`,
            data: { modules: [], model: new VM_Claim() },
            methods: {
                submit() {
...
document.getElementById('detail').onshow = function (args) {
    if (!this.app) {
        this.app = new Vue({
            ...
            data: {
                model: new VM_Claim(),
                get TypeName() { return this.model.GetTypeName(); },
                get StatusName() { return this.model.GetStatusName(); }
            }
        });
    }
    $.get(url + args[0]).then((res) => this.app.model = new VM_Claim(res));
}
document.getElementById('edit').onshow = function (args) {
    if (!this.app) {
        this.app = new Vue({
            el: `#${this.id} .body`,
            data: { model: new VM_Claim() },
            methods: {
                submit() {
...
document.getElementById('users').onshow = function (args) {
    if (!this.app) {
        this.app = new Vue({
            el: `#${this.id} .body`,
            data: { users: [], id: null, model: new VM_UserClaim(), items: [] },
            methods: {
                add() {
                    $.post(`${url + this.id}/Users`, JSON.stringify(this.model)).then(res => {
                        if (res.Success) {
                            $.get(url + args[0] + '/Users').then((res) => this.items = ConvertArrayItems(res, VM_UserClaim));
                        } else {
                            Swal.fire('خطا', res.Message, 'error');
                        }
                    });
    ...
    this.app.id = args[0];
    $.get(url + args[0] + '/Users').then((res) => this.app.items = ConvertArrayItems(res, VM_UserClaim));
}